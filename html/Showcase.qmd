...
\gamma_{is}&=alr_{is}+\left(NPCR\times\alpha_s\right) && \text{(3.3)} \\
\end{align*}
$$
...
$$ 
\Large \begin{align*} 
{\gamma}_{is}&= alr_{is} + NPCR \cdot \alpha \\ 
...
If you remember these equations from Model 4 where everything was relative (proportional) to the reference species and we couldn't escape proportionallity. In Model 5 we use the information from Model 2 to estimate the absolute DNA concentration for the reference species in each environmental sample ()
